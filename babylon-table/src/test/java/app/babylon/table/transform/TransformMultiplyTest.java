...
class TransformMultiplyTest
{
    @Test
    void applyShouldMultiplyDecimalColumns()
    {
        ...
        left.add(new BigDecimal("3.00"));
        ColumnObject.Builder<BigDecimal> right = ColumnObject.builderDecimal(RIGHT);
        right.add(new BigDecimal("1.50"));

        TableColumnar transformed = Tables.newTable(TableName.of("t"), left.build(), right.build())
                .apply(new TransformMultiply(LEFT, RIGHT, TOTAL));

        assertEquals(0, new BigDecimal("4.50").compareTo(transformed.getDecimal(TOTAL).get(0)));
    }
}

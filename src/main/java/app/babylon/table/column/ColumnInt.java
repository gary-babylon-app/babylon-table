...
public interface ColumnInt extends Column
{
    ...
    public static interface Builder extends Column.Builder
    {
        ...
        default Builder add(ParseMode parseMode, ByteSequence bytes, int start, int end)
        {
            ...
            if (parseMode == null || parseMode == ParseMode.EXACT)
            {
                try
                {
                    return add(parser.parseInt(bytes, start, end));
                }
                catch (NumberFormatException e)
                {
                    return addNull();
            ...
            Integer value = parseMode.apply(parser, decoded, 0, decoded.length());
            return value == null ? addNull() : add(value.intValue());
        }
    ...
    @Override
    default public ColumnInt copy(ColumnName x, RowPredicate include)
    {
    ...
    final class Renamed implements ColumnInt
    {
        private final ColumnInt original;
        private final ColumnName name;

        Renamed(ColumnInt original, ColumnName name)
        {

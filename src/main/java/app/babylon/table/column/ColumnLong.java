...
public interface ColumnLong extends Column
{
    ...
    public static interface Builder extends Column.Builder
    {
        ...
        default Builder add(ParseMode parseMode, ByteSequence bytes, int start, int end)
        {
            ...
            if (parseMode == null || parseMode == ParseMode.EXACT)
            {
                try
                {
                    return add(parser.parseLong(bytes, start, end));
                }
                catch (NumberFormatException e)
                {
                    return addNull();
            ...
            Long value = parseMode.apply(parser, decoded, 0, decoded.length());
            return value == null ? addNull() : add(value.longValue());
        }
    ...
    @Override
    default public ColumnLong copy(ColumnName x, RowPredicate include)
    {
    ...
    final class Renamed implements ColumnLong
    {
        private final ColumnLong original;
        private final ColumnName name;

        Renamed(ColumnLong original, ColumnName name)
        {

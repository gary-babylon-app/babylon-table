...
public class HeaderStrategyAutoDetectionTest
{
    ...
    {
        List<RowBuffer> rows = rows(row("Date", "Symbol", "Price", "Volume"),
    ...
    {
        List<RowBuffer> rows = rows(row("Date", "Symbol"), row("2026-01-02", "AAA", "10.25", "1000", "2000"));
        assertEquals(0, HeaderStrategyAuto.detectHeaderRowIndex(rows));
    }
    ...
    {
        List<RowBuffer> rows = rows(row("25569", "25570"), row("2026-03-23", "2026-03-24"),
                row("Date", "Symbol", "Price"), row("2026-01-02", "AAA", "10.25"));
    ...
    {
        List<RowBuffer> rows = rows(row("25570", "25571", "25572"), row("Date", "Symbol", "Close"),
    ...
    {
        List<RowBuffer> rows = rows(row("Date", "", "Price", ""), row("2026-01-02", "", "10.25", ""));
        assertEquals(0, HeaderStrategyAuto.detectHeaderRowIndex(rows));
    }
    ...
    {
        List<RowBuffer> rows = rows(row("Date", "Issuer", "Amount"), row("2026-01-02", "Acme, Inc", "100.00"));
        assertEquals(0, HeaderStrategyAuto.detectHeaderRowIndex(rows));
    }
    ...
    {
        List<RowBuffer> rows = rows(row("TradeDate", "SettlementDate", "Symbol"), row("45500", "45503", "AAA"));
        assertEquals(0, HeaderStrategyAuto.detectHeaderRowIndex(rows));
    }
    ...
    {
        List<RowBuffer> rows = rows(row("ColA", "ColB", "ColC"), row("ColA", "ColB", "ColC"),
                row("2026-01-02", "AAA", "10.25"));
        assertEquals(0, HeaderStrategyAuto.detectHeaderRowIndex(rows));
    }
    ...
    {
        List<RowBuffer> rows = rows(row("Account", "12345678"), row("SortCode", "12-34-56"),
                row("Period", "2026-01-01 to 2026-01-31"), row("Currency", "GBP"),
    ...
    {
        List<RowBuffer> rows = rows(row("Name", "Email", "EmailType", "PhoneType"),
